...
import { personalApi, CATEGORIES_DEPENSES } from '@/services/personalApi'
import type { PersonalDepense } from '@/services/personalApi'
...
const defaultForm = (): FormData => ({
  libelle: '', montant: '', categorie: 'AUTRES',
...
interface ModalProps {
  initial?: PersonalDepense | null
  onSave: (data: FormData) => Promise<void>
  onClose: () => void
}

function DepenseModal({ initial, onSave, onClose }: ModalProps) {
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        <form onSubmit={(e) => { void submit(e) }}>
          <div className="border-b border-gray-100 px-6 py-4">
            <h3 className="font-semibold text-gray-900">{initial ? 'Modifier la dépense' : 'Nouvelle dépense'}</h3>
          </div>
          <div className="space-y-4 px-6 py-5">
            {err && <p className="text-sm text-red-600">{err}</p>}
            <div>
              <label className="label">Libellé *</label>
              <input className="input mt-1" value={form.libelle} onChange={(e) => set(setForm, 'libelle', e.target.value)} placeholder="Ex: Loyer, Courses…" />
            </div>
            ...
            <label className="flex cursor-pointer items-center gap-2">
              ...
              <span className="text-sm text-gray-700">Dépense récurrente (mensuelle)</span>
            </label>
          </div>
...
export function PersonalExpenses() {
  // Période synchronisée avec Dashboard / Income via les query params de l'URL
  const { annee, mois, periodString: filterMois, setPeriod } = usePersonalPeriod()
  const [items, setItems]       = useState<PersonalDepense[]>([])
  const [loading, setLoading]   = useState(true)
  const [filterCat, setFilterCat] = useState('')
  const [modal, setModal]       = useState<'add' | 'edit' | null>(null)
  const [editing, setEditing]   = useState<PersonalDepense | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await personalApi.depenses.list(params)
      setItems(data)
  ...
  async function handleSave(form: FormData) {
    if (editing) {
      await personalApi.depenses.update(editing.id, { libelle: form.libelle, montant: parseFloat(form.montant), categorie: form.categorie, date: form.date, recurrent: form.recurrent })
    } else {
      await personalApi.depenses.create({ libelle: form.libelle, montant: parseFloat(form.montant), categorie: form.categorie, date: form.date, recurrent: form.recurrent })
    }
  ...
  async function handleDelete(id: string) {
    await personalApi.depenses.remove(id)
    setDeleting(null)
    await load()
  }

  const total  = items.reduce((a, i) => a + i.montant, 0)
  const catInfo = (v: string) => CATEGORIES_DEPENSES.find((c) => c.value === v)
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Suivi de vos dépenses personnelles</p>
        </div>
      ...
      {!loading && (
        <div className="flex items-center justify-between rounded-xl bg-red-50 px-5 py-4">
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-red-700">Total dépenses</p>
            <p className="mt-0.5 text-2xl font-bold text-red-600">{fmt(total)}</p>
          </div>
          <p className="text-sm text-red-400">{items.length} opération{items.length > 1 ? 's' : ''}</p>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border-2 border-dashed border-gray-200 py-12 text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">Aucune dépense pour cette période</p>
      ...
      {(modal === 'add' || modal === 'edit') && (
        <DepenseModal initial={editing} onSave={handleSave} onClose={() => { setModal(null); setEditing(null) }} />
      )}

      {deleting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="font-semibold text-gray-900">Supprimer cette dépense ?</h3>
