...
const InvoiceRow = React.memo(function InvoiceRow({ invoice, selected, onSelect, onStatus, onDelete }: RowProps) {
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3 text-sm text-right font-semibold text-gray-900">{fmt(invoice.total)}</td>
...
const RecurringRow = React.memo(function RecurringRow({ rec, onToggle, onGenerate, onDelete }: RecurringRowProps) {
  ...
  return (
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-4 py-3 text-sm text-right font-semibold text-gray-900">{fmt(rec.subtotal)}</td>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="number" step="0.1" min="0" max="100" className="input mt-1"
              value={form.taxRate ?? defaultVatRate} onChange={e => setForm(f => ({ ...f, taxRate: parseFloat(e.target.value) || 0 }))} />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
...
function CreateRecurringModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [form, setForm] = useState<CreateRecurringDto>({
    clientId: '', frequency: 'MONTHLY', subtotal: 0, taxRate: defaultVatRate,
...
function InvoiceList({ selectedId, onSelect, onStatus, onDelete, statusTab, onStatusTab }: InvoiceListProps) {
  const { fmt } = useCurrency()
      ...
      {data && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="card py-3">
            ...
            <p className="mt-1 text-xl font-bold text-gray-900">{fmt(data.totalAmount)}</p>
          </div>
          <div className="card py-3">
            <p className="text-xs text-gray-500">Encaissé</p>
            <p className="mt-1 text-xl font-bold text-green-600">{fmt(data.paidAmount)}</p>
          </div>
          <div className="card py-3">
...
export function BillingPage() {
  ...
  const handleSelect = useCallback((inv: Invoice) => {
    setSelected(s => s?.id === inv.id ? null : inv)
  }, [])
  ...
  const listPane = (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-gray-200">
        {MAIN_TABS.map(t => (
          <button key={t} onClick={() => { setMainTab(t); setSelected(null) }}
