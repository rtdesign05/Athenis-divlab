...
export function AnalyseConcurrentielleDownloadPage() {
  const { downloadAnalyseConcurrentielle } = usePdf()
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 gap-6">
      <div className="bg-white rounded-2xl shadow-md p-10 flex flex-col items-center gap-4 max-w-sm w-full">
        ...
        <h1 className="text-lg font-bold text-gray-900 text-center">Analyse Concurrentielle & Innovations</h1>
        ...
        {status === 'done' && (
          <>
            ...
            <button
              onClick={() => { setStatus('generating'); downloadAnalyseConcurrentielle().then(() => setStatus('done')) }}
        ...
        {status === 'error' && (
          <>
            ...
            <button
              onClick={() => { setStatus('generating'); downloadAnalyseConcurrentielle().then(() => setStatus('done')).catch(() => setStatus('error')) }}
