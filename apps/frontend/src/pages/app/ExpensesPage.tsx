...
interface RowProps { expense: Expense; onDelete: (id: string) => void }
const ExpenseRow = React.memo(function ExpenseRow({ expense, onDelete }: RowProps) {
  return (
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-4 py-3 text-sm text-right text-gray-500">{formatCurrency(expense.tva)}</td>
...
export function ExpensesPage() {
  ...
  const handleDelete = useCallback((id: string) => {
    if (confirm('Supprimer cette dépense ?')) deleteExpense.mutate(id)
  }, [deleteExpense])
        ...
        {data && (
          <div className="grid grid-cols-2 gap-4">
            <div className="card py-3">
              ...
              <p className="mt-1 text-xl font-bold text-gray-900">{formatCurrency(data.totalHT)}</p>
            </div>
            <div className="card py-3">
              <p className="text-xs text-gray-500">TVA déductible</p>
              <p className="mt-1 text-xl font-bold text-gray-700">{formatCurrency(data.totalTVA)}</p>
            </div>
          </div>
        )}
