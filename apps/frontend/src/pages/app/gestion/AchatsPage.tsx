/**
 * AchatsPage — Fix #1 : filtre agences depuis Paramètres
 *
 * Même logique que VentesPage — le dropdown "Agence" est alimenté
 * par la liste réelle venant de Paramètres → Agences.
 */
...
export function AchatsPage() {
  ...
  // Agences depuis Paramètres (réactif)
                ...
                ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-400">
                      {agenceFilter !== 'all' && !agenceNom
                        ? `Aucun achat pour « ${agenceFilter} »`
                        : 'Aucun achat pour cette agence'}
                    </td>
                  </tr>
                )
                : achats.map(c => (
