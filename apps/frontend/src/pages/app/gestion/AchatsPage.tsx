...
function FournisseursTab({ agenceNom }: { agenceNom: string | null }) {
  ...
  function handleSave(data: Omit<Fournisseur, 'id' | 'createdAt'>) {
    if (editing) {
      updateFournisseur(editing.id, data)
      setEditing(null)
    } else {
      addFournisseur(data)
      setShowModal(false)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { deleteFournisseur(confirmDel); setConfirmDel(null) }}
...
export function AchatsPage() {
  ...
  return (
    <div className="h-full flex flex-col gap-3">
      ...
      {/* ── En-tête : titre + onglets + filtres sur une seule ligne ── */}
      ...
      {tab === 'commandes' ? (
        ...
        <FournisseursTab agenceNom={agenceNom} />
      )}
    </div>
  )
}
