...
interface ModalBLProps {
  ...
  onSave: (data: { commande: string; client: string; agence: string; dateCreation: string; datePrevue: string; dateLivraison: null; statut: BLStatut }) => void
  onClose: () => void
}

function ModalBL({ commandes, agenceNom, onSave, onClose }: ModalBLProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3">
          ...
          <p className="text-[11px] text-gray-400">Statut initial : <strong>En préparation</strong></p>
...
export function BonsLivraisonPage() {
  const { user } = useAuth()
  const { bonsLivraison, commandes, updateBLStatut, addBonLivraison } = useGestion()
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {items.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-sm text-gray-400">Aucun bon de livraison trouvé</td></tr>
              ) : items.map(b => (
